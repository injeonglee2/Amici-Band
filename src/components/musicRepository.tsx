...
export const MUSIC_FOLDER_CONFIG: FolderModuleConfig = {
  templates: [
    ...
    { id: 'recommendation', label: '추천곡', description: '추천 → 보컬·악기 투표 → 선정', symbol: '💡' },
  ...
  filters: [
    ...
    { id: 'rehearsal', label: '합주곡', match: (folder) => folder.templateId === 'rehearsal', openFolderId: REHEARSAL_PLAYLIST_ID },
  ...
  canCreate: (filterId) => filterId !== 'rehearsal',
  labels: {
  ...
  rowIcon: (folder) => <span className="folder-emoji">{folder.templateId === 'rehearsal' ? '🎼' : folder.templateId === 'recommendation' || folder.templateId === 'project' ? '💡' : folder.templateId === 'performance' ? '🎸' : '🎧'}</span>,
}
...
export const playlistRepository: FolderRepository<Playlist> = {
  ...
  playlistImport: {
    templateIds: ['general', 'recommendation'],
