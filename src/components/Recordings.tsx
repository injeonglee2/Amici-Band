...
function RecordingForm({ editing, folderId, existingVideoIds, toast, onClose }: { editing: Recording | null; folderId?: string; existingVideoIds: Set<string>; toast: ToastState; onClose: () => void }) {
  ...
  useEffect(
    () => watchEvents((list) => setEvents(member?.admin ? list : list.filter((e) => !e.adminOnly && (!e.targetMemberIds?.length || (!!member?.uid && e.targetMemberIds.includes(member.uid))))), () => {}),
    [member?.admin, member?.uid],
  )
...
export function RecordingPlayer({ rec, toast, onEdit, onClose, readOnly }: { rec: Recording; toast: ToastState; onEdit?: () => void; onClose: () => void; readOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    return watchEvents((evs) => {
      if (!member?.admin) evs = evs.filter((e) => !e.adminOnly && (!e.targetMemberIds?.length || (!!member?.uid && e.targetMemberIds.includes(member.uid))))
      const e = evs.find((x) => x.id === rec.eventId)
      setLiveEventTitle(e ? e.title : null)
    }, () => {})
  }, [rec.eventId, member?.admin, member?.uid])
...
function FolderSyncSheet({ folder, existingVideoIds, member, toast, onClose }: {
  ...
  async function unlink(pid: string) {
    try {
      await removeRecordingFolderPlaylist(folder.id, pid)
      setPls((current) => current.filter((id) => id !== pid))
      setUnlinkTarget(null)
      toast.show('연결을 해제했어요')
    } catch { toast.show('해제하지 못했어요') }
  }
  async function addPlaylist() {
    const pid = parsePlaylistId(playlistUrl)
    if (!pid) { setAddError('올바른 유튜브 재생목록 링크를 입력해 주세요.'); return }
    if (pls.includes(pid)) { setAddError('이미 연결된 재생목록이에요.'); return }
    ...
    } catch { setAddError('재생목록을 연결하지 못했어요.') }
